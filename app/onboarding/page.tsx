  ...
  return (
    <main className="min-h-screen bg-slate-950 px-4 py-8 text-white">
      <div className="mx-auto max-w-2xl">
        <div className="mb-8 flex items-center justify-between"><b>💰 Sai do Vermelho</b><span className="text-sm text-slate-400">Etapa {step} de 5</span></div>
        ...
        <section className="rounded-3xl bg-white p-6 text-slate-900 shadow-2xl md:p-9">
          {step === 1 && <>
            <p className="text-sm font-bold text-blue-700">VAMOS COMEÇAR</p>
            <h1 className="mt-2 text-3xl font-bold">O que você mais quer conquistar?</h1>
            <p className="mt-2 text-gray-600">Isso nos ajuda a organizar sua jornada na ordem certa.</p>
            <div className="mt-6 grid gap-3 sm:grid-cols-2">{goals.map((item) => <button key={item.value} onClick={() => setGoal(item.value)} className={`rounded-2xl border-2 p-4 text-left ${goal === item.value ? 'border-blue-600 bg-blue-50' : 'border-gray-200'}`}><span className="text-2xl">{item.icon}</span><p className="mt-2 font-bold">{item.label}</p></button>)}</div>
          </>}
          {step === 2 && <>
            <p className="text-sm font-bold text-blue-700">SUA RENDA</p><h1 className="mt-2 text-3xl font-bold">Quanto realmente entra por mês?</h1>
            <div className="mt-6 grid grid-cols-2 gap-3">
              <button onClick={() => setIncomeType('fixed')} className={`rounded-xl border-2 p-3 ${incomeType === 'fixed' ? 'border-blue-600 bg-blue-50' : 'border-gray-200'}`}>Renda fixa</button>
              <button onClick={() => setIncomeType('variable')} className={`rounded-xl border-2 p-3 ${incomeType === 'variable' ? 'border-blue-600 bg-blue-50' : 'border-gray-200'}`}>Renda variável</button>
            </div>
          ...
          {step === 3 && <>
            <p className="text-sm font-bold text-blue-700">SEU ESSENCIAL</p><h1 className="mt-2 text-3xl font-bold">Quanto custa manter sua casa funcionando?</h1>
          ...
          {step === 4 && <>
            <p className="text-sm font-bold text-blue-700">SUAS DÍVIDAS</p><h1 className="mt-2 text-3xl font-bold">Existe alguma dívida para organizar?</h1>
          ...
          {step === 5 && <>
            <p className="text-sm font-bold text-emerald-700">SEU PLANO ESTÁ PRONTO</p><h1 className="mt-2 text-3xl font-bold">A partir de agora, você não caminha sem direção</h1>
